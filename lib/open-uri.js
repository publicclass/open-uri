...
open.ftp = function(uri,opts,output){
  ...
  if( !uri.userinfo && opts.anonymous )
    uri.userinfo = "anonymous";
  ...
  client.on("connect",function request(err){
    ...
    if( uri.userinfo && !authenticated )
      return client.auth(uri.username,uri.password,function(err){authenticated=!err;request(err)})
...
function error(output,err){
  typeof output == "function" ? output(err) : throw( err );
  return open;
}
